...
- **Long-Session Context Policy & Context Governor System** (`crates/ghost-link/src/main.rs`, `crates/ghost-link/src/native_engine.rs`, `ghostlink_gui_modern/src/store.ts`, `ghostlink_gui_modern/src/components/SettingsTab.tsx`, `ghostlink_gui_modern/src/components/ChatTab.tsx`, `docs/LOCAL_INFERENCE_TUNING.md`):
  Implemented a real long-session context policy and Context Governor on the live inference path to support multi-hour chat threads without silent truncation or VRAM OOM. Added typed policy settings (`n_ctx`, `n_ctx_auto`, `kv_cache_type`, `conversation_token_limit`, `max_tokens`, `policy`, `keep_last_turns`, `reserve_completion_ratio`, `sticky_slot`) across `RuntimeSettings`, `/api/settings`, and GUI `SettingsTab.tsx`. Updated `native_engine.rs` `get_ctx_size_budgeted` to remove the hard 4096-token cap for >= 10GB models when distributed RPC inference or aggregate cluster memory is sufficient, returning structured memory fit errors on model load when explicit context sizes exceed available capacity. Added `apply_context_governor_async` on the chat path to estimate prompt tokens, reserve completion budget, apply governor policies (`sliding_window`, `truncate_oldest`, `compact`), and handle sticky slot cache reuse (`id_slot`). Added compact memory summarization, Context Meter UI in `ChatTab.tsx`, and IndexedDB thread storage removing the silent 200-message cap.
